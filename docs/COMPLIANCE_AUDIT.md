...
|---|-------|----------|------|---------|----------------|
| 1 | **`ArticleModel` and `AiInsightModel` use `fromJson` not `fromRawData`** — Retrofit code generation mandates `fromJson` for `ArticleModel`; `AiInsightModel` uses `fromJson` as the standard JSON-parsing convention (Firestore + Gemini API both produce JSON maps). | **Low** | AV 1.3.3 | `daily_news/data/models/article.dart`, `ai_insight/data/models/ai_insight_model.dart` | Document as justified deviation: framework/API constraints. `FirebaseArticleModel` uses `fromRawData` to demonstrate awareness of the convention. |
